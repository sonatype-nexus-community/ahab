    ...
    vulnerabilityScan: {
      withDockerImage(env.DOCKER_IMAGE_ID, {
        withCredentials([usernamePassword(credentialsId: 'jenkins-saas-service-acct',
          ...
          sh 'go list -json -deps | /tmp/tools/nancy iq --iq-application ahab --iq-stage release --iq-username $IQ_USERNAME --iq-token $IQ_PASSWORD --iq-server-url https://sonatype.sonatype.app/platform'
        }
      })
    },
    onFailure: {
